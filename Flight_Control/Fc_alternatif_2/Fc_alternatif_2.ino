...
double pid[2], pwmLeft, pwmRight, pwmFront, pwmBack, error_roll, error_pitch, previous_error_roll, previous_error_pitch;
float pid_p[2] = {0, 0};
float pid_i[2] = {0, 0};
float pid_d[2] = {0, 0};
///////////////////PID CONSTANTS/////////////////
...
double kd[2] = {0.4, 0.4}; //2.05

//double kp[2]={0, 0};//3.55
//double ki[2]={0, 0.000};//0.003
//double kd[2]={0, 0.0};//2.05
///////////////////////////////////////////////
double throttle_value = 1400; //
...
PID Roll_PID(&error_roll, &pid[0], &desired_angle,  kp[0], ki[0], kd[0], REVERSE);    //Roll
PID Pitch_PID(&error_pitch, &pid[1], &desired_angle, kp[1], ki[1], kd[1], REVERSE);    //Pitch
  ...
  if (Serial.available() > 0) {
    ...
    else if (incomingByte == 'p')
    {
      ...
      while (1)
      {
        ...
        kp[0] = incomingValue;
        kp[1] = incomingValue;
        Roll_PID.SetTunings(incomingValue, ki[0], kd[0]);
        Pitch_PID.SetTunings(incomingValue, ki[1], kd[1]);
        Motor_Loop();
        break;
        }
      }
    ...
    else if (incomingByte == 'i')
     {
      ...
      while (1)
      {
        ...
        ki[0] = incomingValue;
        ki[1] = incomingValue;
        Roll_PID.SetTunings(kd[0], incomingValue, kd[0]);
        Pitch_PID.SetTunings(kd[1], incomingValue, kd[1]);
        incomingValue = 0;
        Motor_Loop();
        break;
        }
    ...
    else if (incomingByte == 'd')
     {
      ...
      while (1)
      {
        ...
        kd[0] = incomingValue;
        kd[1] = incomingValue;
        Roll_PID.SetTunings(kd[0],ki[0] , incomingValue);
        Pitch_PID.SetTunings(kd[1], ki[1], incomingValue);
        incomingValue = 0;
        Motor_Loop();
        break; 
        }
